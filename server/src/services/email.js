...
/** Build an absolute download URL for a sale. */
export function buildDownloadLinks(sale) {
  return {
    master: `${config.appUrl}/api/download/${sale.id}/${sale.downloadToken}?file=master`,
    stems: sale.includesStems
      ? `${config.appUrl}/api/download/${sale.id}/${sale.downloadToken}?file=stems`
      : null,
  };
}
...
/**
 * Exclusive sale email — beat + stems links + license document.
 */
export async function sendExclusiveSaleEmail(sale, beat) {
  const links = buildDownloadLinks(sale);

  // PDF license document (attached) — also rendered inline below
  const pdfBuffer = await licensePdfBuffer({ sale, beat });
  const pdfAttachment = pdfBuffer.toString('base64');

  const body = `
    <p>Congratulations! You now own <strong>"${escapeHtml(sale.beatTitle)}"</strong> exclusively. 🎉</p>
    <p>Your payment of <strong>$${sale.amountUsd.toFixed(2)}</strong> has been confirmed and the beat has been <strong>removed from the marketplace</strong>. It belongs only to you now.</p>
  ...
  const { data, error } = await resend.emails.send({
    ...
    attachments: [
      {
        filename: `WISHAM-Exclusive-License-${sale.beatTitle.replace(/[^a-z0-9]+/gi, '-')}.pdf`,
...
/**
 * Inclusive sale email — download links only, NO license.
 */
export async function sendInclusiveSaleEmail(sale) {
  ...
  const body = `
    ...
    <p>Payment confirmed: <strong>$${sale.amountUsd.toFixed(2)}</strong>.</p>
